...
void draw() { 
  if (!glitchSaved)
  {
    ...
    {
      println("iterate img "+num+" "+iterations+" times.");
      ...
      if(shiftHorizontally)
        horizontalShift = int(random(targetImg.width/40));
      ...
      if(shiftVertically)
        verticalShift = int(random(targetImg.height/40));
  ...
  if (!glitchSaved) {
    ...
    // save surface
    targetImg.save(imgFileName+"_"+num+"_glitched.png");
    num++;
    iterations++;
    if (num == 30) {
      glitchSaved = true;
